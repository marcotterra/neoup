$(document).ready(function($) {
	...
	sendEmail.click(function(event){
		...
		var comment = $('#to_comment').val();

		if(!name || name.length == 0 || !email || email.length == 0) {alert('prencha os campos'); return}
		
		var data = {
			name: $('#to_name').val(),
			email: $('#to_email').val(),
			comment: $('#to_comment').val()
		}
		...
		req.open("post", "http://localhost:8000/email", true);
